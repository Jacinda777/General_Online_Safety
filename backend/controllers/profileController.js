...
const saveProfile = async (req, res) => {
    try {
        const { name, email, bio, phone, avatar, twoFactorEnabled, receiveNotifications, stats } = req.body;
        ...
        user.receiveNotifications = receiveNotifications !== undefined ? receiveNotifications : user.receiveNotifications;
        ...
        res.set('Cache-Control', 'no-store');
        res.status(500).json({ success: false, message: `Failed to save profile: ${error.message}` });
    }
};
...
const updateSecuritySettings = async (req, res) => {
    try {
        ...
        res.set('Cache-Control', 'no-store');
        res.status(500).json({ success: false, message: `Failed to update security settings: ${error.message}` });
    }
};
